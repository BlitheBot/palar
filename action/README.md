...
| --- | --- | --- |
| `version` | `"0.5.0"` | Version of `palar` to install — pinned so the gate's verdict cannot change without a change in your repo; set it to `^0.5.0` or `latest` to float. Ignored when `binary` is set |
