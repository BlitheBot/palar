...
looks risky. `palar live` goes further. It starts the server in a locked-down
Docker container, sends crafted input to its tools, and waits for a callback to
a listener on your machine. If the callback arrives, the injection ran. A
...
Requires Docker, installed and running. The first run builds a sandbox image
(~300 MB), which takes a few minutes and isn't counted against the scan timeout.
...
- Sandboxed targets must be Node servers installed on disk. Python, uvx,
  pipx, and `npx -y` targets are refused up front with the reason. A server
  that's already running can be enumerated with `palar scan --from-url`.
...
| --- | --- | --- |
| `schema-integrity` | IV-001, IV-002, TA-101 | Potentially sensitive string inputs (`command`, `path`, `url`, `sql`, …) whose schema does not narrow the values they accept (no `enum`, `const`, or non-trivial `pattern`); sensitive-named tools with no input schema at all. **IV-001 is `medium` and explicitly a hypothesis** — see "Static is a hypothesis, the oracle is evidence" below. **TA-101 is live-only**: a tool whose declared MCP annotations (`readOnlyHint`, `destructiveHint`, `openWorldHint`) are refuted by a probe callback — the claim read from the server's own `listTools()`, the behaviour proven by a nonce that came back. A hint the server never declared is never contradicted |
| `text-sanitization` | TS-001…TS-006 | Hidden Unicode in a tool's three display surfaces — `name`, `description`, and `title` (both the top-level position and `annotations.title`, since both are in active use): zero-width characters, bidi override controls, tag characters, stray variation selectors, non-printable controls — reported by code point, never echoed. Plus TS-006, mixed-script and NFKC-compatibility look-alikes in a tool NAME |
